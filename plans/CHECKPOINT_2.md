...
A snapshot of where `without` stands, succeeding `CHECKPOINT_1.md`. For the
...
- **Graph/DAG recovery dropped for now.** The `@node` decorator, `Registry`,
  topological order, and mermaid visualizer were prototyped then set aside to
  keep the focus on the streams core. The idea still stands (it is one of the
  three bets in `BIG_IDEA.md`); when it returns it should be built on stdlib
  `graphlib.TopologicalSorter`, which will also help actually executing a graph.
- **Naming settled** on the behavior edge: the holder is a public `Sample` with
  a private `_value` and a `current()` read method (the view that makes every
  context interchangeable). The task-scoping helper is `background_task`.
- **ConfigMap parses a single YAML file** (`read_yaml_file`) rather than a
  directory of per-key files.
- **Test helpers** live in `without.testing` (`stream`, `collect`, `tick`), and
  the cross-plugin test in `without-integration` now exercises two plugins
  together (a static env context and a reloading configmap context coexisting).
...
- **Two edge types (events vs. behaviors).** An *event* edge feeds every output
  ...
  something running (a `background_task`), so it is the visible seam where the
  imperative shell shows up.
...
- `packages/without` (import `without`) — the core. `contracts` (`Processor`,
  ...
  `pipe`, `sample`), `tasks` (`background_task`, a `with`-scoped background
  task), and `testing` (`stream`, `collect`, `tick`). Zero runtime dependencies.
...
Done and verified (mypy strict clean across 17 source files, 14 tests passing,
ruff lint + format clean):

- The core contract: stream-to-stream processors with the pure reducer as the
  testable kernel.
- The two edge connectors, `pipe` and `sample`, plus `background_task`.
- Two plugins: env vars (static context) and ConfigMap (changing context),
  proving both halves of the events/behaviors model end to end, and a
  cross-plugin test showing them coexist.
...
- **Graph/DAG recovery and visualization**, rebuilt on `graphlib`.
- Known-hard problems inherited from dataflow/FRP: backpressure on event edges,
  glitches on diamond dependencies, and feedback cycles and teardown order.
